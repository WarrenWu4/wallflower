...
class HyprpaprParser {
private:
  HyprpaprConfig config;
  ...
  void parseWallpaper(const std::string &value) {
    ...
    if (!preloaded) {
      throw std::runtime_error("Wallpaper image path not preloaded: " + wp.imagePath);
    }
    config.wallpaper.push_back(wp);
  }

public:
  HyprpaprParser(const std::string &path): configPath(path) {
    config.preload = {};
    config.wallpaper = {};
    config.splash = false;
